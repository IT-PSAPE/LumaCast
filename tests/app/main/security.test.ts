...
// tests/app/main/application-menu.test.ts.
vi.mock('electron', () => ({
...
} from '../../../app/main/security';

// The packaged renderer path must come out identical to the one security.ts
// derives for PACKAGED_RENDERER_INDEX_PATH (and that index.ts's
// loadRendererView loads), which security.ts resolves as `../renderer` from
// its own app/main __dirname. This file lives under tests/, so it walks back
// up to the repo root and down into app/renderer to land on the same path.
const PACKAGED_RENDERER_URL = pathToFileURL(path.join(__dirname, '../../../app/renderer/index.html')).toString();
