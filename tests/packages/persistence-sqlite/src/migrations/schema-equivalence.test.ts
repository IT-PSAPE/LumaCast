...
// `fileURLToPath(new URL('../../../../../packages/persistence-sqlite/src/', import.meta.url))` pattern used below to
...
const EXPECTED_SCHEMA_VERSIONS = Array.from({ length: 37 }, (_, i) => i);
...
describe('schema equivalence across historical fixtures (#108)', () => {
  it('pins fixture manifests for exactly versions 0..36 with matching directory suffixes', () => {
    expect(LATEST_SCHEMA_VERSION, 'LATEST_SCHEMA_VERSION must be 36').toBe(36);
    expect(
      MIGRATIONS.map((migration) => migration.version),
      'MIGRATIONS versions must be a dense contiguous prefix 1..36',
    ).toEqual(Array.from({ length: 36 }, (_, i) => i + 1));
    expect(
      FIXTURES.map((fixture) => fixture.version),
      'fixture manifests must cover exactly versions 0..36 with no gaps or duplicates',
