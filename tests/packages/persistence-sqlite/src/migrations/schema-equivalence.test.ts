...
const EXPECTED_SCHEMA_VERSIONS = Array.from({ length: 35 }, (_, i) => i);
...
describe('schema equivalence across historical fixtures (#108)', () => {
  it('pins fixture manifests for exactly versions 0..34 with matching directory suffixes', () => {
    expect(LATEST_SCHEMA_VERSION, 'LATEST_SCHEMA_VERSION must be 34').toBe(34);
    expect(
      MIGRATIONS.map((migration) => migration.version),
      'MIGRATIONS versions must be a dense contiguous prefix 1..34',
    ).toEqual(Array.from({ length: 34 }, (_, i) => i + 1));
    expect(
      FIXTURES.map((fixture) => fixture.version),
      'fixture manifests must cover exactly versions 0..34 with no gaps or duplicates',
