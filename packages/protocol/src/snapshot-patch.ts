...
export function applyPatch(snapshot: AppSnapshot, patch: SnapshotPatch): AppSnapshot {
  const next: AppSnapshot = {
    ...
    timers: mergeTable(snapshot.timers ?? [], patch.upserts.timers, patch.deletes.timers),
  };
  return next;
}
...
export function invertPatch(snapshot: AppSnapshot, patch: SnapshotPatch): SnapshotPatch {
  ...
  invertTable(snapshot.timers ?? [], patch.upserts.timers, patch.deletes.timers, inverse, 'timers');
...
function appendInverseUpsert<K extends SnapshotTableKey>(
  ...
  switch (key) {
    ...
    case 'slideTags':
      ...
      return;
    case 'timers':
      inverse.upserts.timers = [...(inverse.upserts.timers ?? []), value as Timer];
      return;
  }
}
...
function appendInverseDelete(inverse: SnapshotPatch, key: SnapshotTableKey, id: Id): void {
  switch (key) {
    ...
    case 'slideTags':
      ...
      return;
    case 'timers':
      inverse.deletes.timers = [...(inverse.deletes.timers ?? []), id];
      return;
  }
}
