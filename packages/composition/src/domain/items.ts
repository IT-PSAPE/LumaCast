...
// refactor decision D1): the two independent item entities — Presentation,
// Lyric — plus the discriminant types code uses to describe "one of
// the two" without resurrecting a union. There is no shared exported base
// interface and no union entity: the two are separate tables in practice
// and separate, independent interfaces here.
...
/**
 * A typed pointer to one of the two items: `type` says which table/map to
 * resolve `id` against. Used wherever code structurally needs "one of
 * presentation | lyric" — selection, playback arming, macro scope
 * contexts, drag payloads — without merging the two id spaces into one.
 */
