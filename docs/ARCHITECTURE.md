...
- `packages/persistence-sqlite/src/migrations/definitions.ts` is the canonical dense ordered migration list, v1..v34; v1 is the bootstrap. Both a fresh v0 database and any historical database advance through the same `runMigrations` path (`packages/persistence-sqlite/src/migrations/runner.ts`); there is no separate fresh-install schema.
...
- Fixtures `schema-v0`..`schema-v34` pin frozen structural fingerprints and convergence coverage for every historical version; they are regression evidence, not a second schema definition. Migrations v1–v22 and their fixtures are frozen and never edited; every schema change since is a new migration.
...
- Round-trip tests verify background and recursive element provenance survive restart and snapshot restore. Bundle export/import specifically is covered by `tests/packages/persistence-sqlite/src/theme-provenance.test.ts`: `finalizeImportBundle` re-materializes every imported theme with new element IDs, so it translates each item element's `sourceThemeElementId` through a map from pre-import to newly materialized theme element IDs built during the theme-import pass; an ID that does not resolve through that map (dangling, unknown, or naming a theme not assigned to that item) is written as `null` rather than passed through, since an unresolved ID would otherwise read as "source removed from the theme" and be deleted on the next sync (see ADR-0003).

## Slide tags

- `SlideTag` is project-level metadata in `@lumacast/composition`: `{ id, name, colorKey, order, createdAt, updatedAt }`. Its color key uses the same named palette as playlist separators. A content slide stores at most one nullable `tag_id`; theme, overlay, and stage container slides are not tag targets.
- Tags do not create or modify canvas elements. The renderer resolves `Slide.tagId` against `AppSnapshot.slideTags` and colors the existing slide caption surface. Editing a tag therefore updates every assigned caption through the normal snapshot patch without rewriting its slides.
- `createSlideTag`, `updateSlideTag`, `deleteSlideTag`, and `assignSlideTags` cross the typed IPC boundary and return ordinary `SnapshotPatch` values. Assignment accepts several slide IDs in one transaction. Deleting a tag clears every referencing slide before deleting the definition; the patch contains both the cleared slides and deleted tag.
- Migration v34 adds `slide_tags` and the nullable `slides.tag_id` foreign key. Full snapshots, incremental patches, item/slide duplication, and project backup/recovery all preserve assignments. Project backups restore tag definitions before slides and validate every non-null `tag_id` before touching the active database. See ADR-0034.

## Timers

- `Timer` is a global named entity in `@lumacast/composition` (`packages/composition/src/domain/timers.ts`), not slide-scoped metadata: one of three kinds (countdown, countdown-to-time, elapsed), plus `allowOverrun` (keep counting past zero/the end instead of clamping), a `TimerFormat` (`mm:ss` / `hh:mm:ss`), and an ordered set of colour `thresholds` that recolour any text linked to the timer once its value crosses them.
- Run state (`TimerRunState`: idle/running/paused, `startedAtMs`, `accumulatedMs`) is volatile. It lives only in the renderer's `TimersProvider` and is never persisted — the app is single-window, so there is no cross-window state to share and no restart continuity to preserve.
- The provider ticks and resolves every timer's `TimerReading` (`resolveTimerReading`) from its config and run state. Readings reach the canvas through `BindingValue.timerReadings` (keyed by `Timer.id`), which replaces the old per-binding `armedAtMs`.
- A text element links to a timer by id (`TextBinding.timerId`; inspector tab "Text link", formerly "Binding"). `useResolvedText` looks up the reading for `kind: 'timer'`: no reading (never linked, or the timer was deleted) renders `--:--`; an active threshold's colour overrides the text fill.
- The Timers panel, a tab in the program panel, is the only control surface: create/rename/duplicate/delete timers, edit kind-specific fields and thresholds, and start/pause/reset. `timer.create`/`timer.update`/`timer.delete` are ordinary main-process mutations returning a `SnapshotPatch`, mirroring `slideTag.*`. `timer.start`/`timer.stop`/`timer.reset`/`timer.resetAll` are renderer-executed actions (ADR-0037) against the provider's state, reachable from the agent/MCP layer without a main-process round trip.
- Migration v36 adds the `timers` table and converts legacy `TextBinding` timer fields (`timerDurationSeconds`/`timerFormat`) into a real `Timer` row plus a `timerId` link, one row per distinct (duration, format) pair. The scan covers only `slide_elements.payload_json`: themes, overlays, and stages have no `elements_json` column of their own and store their elements as `slide_elements` rows against a synthetic container slide, so that one table already holds every owner's bindings. The migration also retires the old stage-arming clock (`resolveStageArmedAt`, a single global `armedAtMs` set on stage take) that every timer binding previously shared, with no per-timer controls. See ADR-0042.

## Project Backup (format v3)
...
- Envelope (`ProjectBackup`): `{ format: 'cast-project-backup', version: 3, schemaVersion: 34, tables }`. `schemaVersion` is the source `PRAGMA user_version`; schemas 30–33 are normalized on import to add media metadata, playback schedules, theme override keys, the post-Talk schema, and slide-tag defaults as needed. There are no settings/preferences tables, so migration/schema metadata is carried by `schemaVersion` alone. The envelope carries no timestamp, so two exports of unchanged data are deeply and byte-for-byte identical.
- `PROJECT_BACKUP_VERSION` is 3. `tables` contains 20 application-owned tables — `presentations`, `lyrics`, `slides`, `slide_elements`, `slide_tags`, `playlists`, `playlist_entries`, `image_assets`, `video_assets`, `audio_assets`, `overlays`, `presentation_themes`, `lyric_themes`, `overlay_themes`, `stages`, `cues`, `actions`, `action_steps`, `trigger_bindings`, `playback_schedules` — with no Talk, library, playlist-group, collection, or unified-theme tables. `playlist_entries` rows are flat and `kind`-discriminated (`'item' | 'separator'`). Every row field is constructed explicitly from the SQL columns in deterministic order (`ORDER BY created_at ASC, id ASC`); JSON-valued columns are serialized as raw JSON strings.
- **Version 1 backups (schemaVersion exactly 22) are imported via migration replay**, not a hand-written transform: `restoreProjectBackup` validates the frozen v1 contract, materializes it in a throwaway SQLite database at schema 22, and replays the real migrations v23–v34 before feeding the result through the ordinary v3 restore path. `validateProjectBackup` remains current-format-only and rejects version 1 with an "older app version" message.
...
- `restoreFromSnapshot` clears every application-owned table and re-inserts the snapshot's rows in one transaction, child-before-parent on delete and parent-before-child on insert (the same discipline `clearProjectBackupTables`/`insertProjectBackupRows` use): the three theme tables, slide tags, and `presentations`/`lyrics` are deleted after `slides` and re-inserted before it; `playlists` and `playlist_entries` are ordinary flat tables restored the same way as every other table — there is no special-cased tree-replacement machinery (the old `libraryBundles` full-replacement patch key is gone with it).
