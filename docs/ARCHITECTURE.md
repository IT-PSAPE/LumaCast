...
- Two adjacent, unrelated bugs in the same method were fixed alongside it, both previously masked because the missing-collection-row FK error always threw first: `insertSlide`'s `VALUES` clause was missing a placeholder (15 for 16 columns), and the generic slide-element restore loop iterated `snapshot.slideElements` unfiltered. That array (from `getSlideElements()`) is not scoped to deck content slides the way `snapshot.slides` (from `getSlides()`) is — it also carries every theme/overlay/stage container's elements, which are already restored separately via `replaceContainerElements` in the theme/overlay/stage loops. The fix restricts the generic loop to elements whose `slideId` is one of the just-inserted deck content slides.